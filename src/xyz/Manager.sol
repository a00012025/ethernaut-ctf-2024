...
contract Manager is Ownable {
    ...
    event LiquidateWithHealth(uint256 health);
    ...
    function manage(
        ...
        if (
            address(collateralData[token].protocolCollateralToken) == address(0)
        ) {
            revert CollateralTokenNotAdded();
        }

        if (
            positionCollateral[msg.sender] != IERC20(address(0)) &&
        ...
        ERC20Signal protocolCollateralToken = collateralTokenInfo
            .protocolCollateralToken;
        ...
        if (
            !debtIncrease &&
            (debtDelta == type(uint256).max ||
                (debtBefore != 0 && debtDelta == debtBefore))
        ) {
    ...
    function liquidate(address liquidatee) external {
        ...
        uint256 health = ProtocolMath._computeHealth(
            wholeCollateral,
            wholeDebt,
            price
        );
        ...
        emit LiquidateWithHealth(health);
    ...
    function addCollateralToken(
        ...
        ERC20Signal protocolCollateralToken = new ERC20Signal(
            ...
            string(
                bytes.concat(
                    "XYZ ",
                    bytes(IERC20Metadata(address(token)).name()),
                    " collateral"
                )
            ),
            string(
                bytes.concat(
                    "xyz",
                    bytes(IERC20Metadata(address(token)).symbol()),
                    "-c"
                )
            )
        );
        ...
        protocolCollateralTokenInfo
            .protocolCollateralToken = protocolCollateralToken;
    ...
    function _updateOperationTime(IERC20 token) internal {
        uint256 pastTime = block.timestamp -
            collateralData[token].operationTime;
    ...
    function _calcDecayedRate(IERC20 token) internal view returns (uint256) {
        uint256 pastMinutes = (block.timestamp -
            collateralData[token].operationTime) / 1 minutes;
