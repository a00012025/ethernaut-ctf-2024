...
contract ERC20Signal is ERC20, ManagerAccess {
    ...
    ) ERC20(_name, _symbol) ManagerAccess(_manager) {
        signal = _signal;
    }
    ...
    function mint(address to, uint256 amount) external onlyManager {
        ...
        // mint amount: transfer in: amount.divUp(signal)
        // later want to burn: got balanceOf: amount.divUp(signal).mulDown(signal')
        //  -> transfer out: amount.divUp(signal).mulDown(signal').divUp(signal')
        // XYZ balance change: amount - amount.divUp(signal).mulDown(signal')
        // target: signal of debt decreases!
    }
    ...
    function setSignal(uint256 backingAmount) external onlyManager {
        ...
        uint256 newSignal = (backingAmount == 0 && supply == 0)
            ? ProtocolMath.ONE
