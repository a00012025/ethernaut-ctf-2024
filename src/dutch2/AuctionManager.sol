...
contract AuctionManager {
    ...
    function create(
        ...
        // start < end <= startVesting <= endVesting
        ...
        // minBid * (2**128-1) / totalBase <= resQuoteBase (?)
        if (
            (params.minBid).mulDivDown(type(uint128).max, params.totalBase) >
            params.resQuoteBase
        ) revert();
        ...
        uint256 beforeBalance = ERC20(params.tokenBase).balanceOf(
            address(this)
        );

        ERC20(params.tokenBase).safeTransferFrom(
            msg.sender,
            address(this),
            params.totalBase
        );
    ...
    function addBid(
        ...
        if (
            amountQuote < auction.parameters.minBid ||
            amountQuote == 0 ||
            amountQuote == type(uint128).max
        ) revert();
        ...
        ERC20(auction.parameters.tokenQuote).safeTransferFrom(
            msg.sender,
            address(this),
            amountQuote
        );
    ...
    function show(
        ...
        uint256 privateKey,
        bytes calldata data
    ) external checkState(States.Reveal, auctions[id]) {
        Auction storage auction = auctions[id];
        if (auction.data.seller != msg.sender) revert();

        Math.Point memory publicKey = Math.publicKey(privateKey);
        if (
            publicKey.x != auction.parameters.publicKey.x ||
            publicKey.y != auction.parameters.publicKey.y ||
        ...
        // possible issue?
        if (0 != data.length) {
            (uint256[] memory indices, uint128 base, uint128 quote) = abi
                .decode(data, (uint256[], uint128, uint128));
    ...
    function finalize(
        ...
        if (data.totalBase != data.baseFilled) {
            ...
            ERC20(auction.parameters.tokenBase).safeTransfer(
                auction.data.seller,
                data.totalBase - data.baseFilled
            );
        }
    ...
    function refund(
        uint256 id,
        uint256 index
    ) external checkState(States.Final, auctions[id]) {
        Auction storage auction = auctions[id];
        BidEncrypted storage bid = auction.bids[index];
    ...
    function withdraw(
        ...
        if (bid.amountQuote != 0) {
            uint256 quoteBought = amountBase.mulDivDown(
                auction.data.quoteLowest,
                auction.data.baseLowest
            );
            bid.amountQuote = 0;
    ...
    function bidCancel(uint256 id, uint256 index) external {
        ...
        if (block.timestamp >= auction.time.end) {
            if (
                block.timestamp <= auction.time.end + 24 hours ||
                auction.data.quoteLowest != type(uint128).max
            ) {
                revert();
            }
        }
    ...
    function tokensAvailableForWithdrawal(
        uint256 id,
        uint128 amountBase
    ) public view returns (uint128 tokensAvailable) {
        Auction memory auction = auctions[id];
        return
            Math.availableTokensAtTime(
                auction.time.startVesting,
                auction.time.endVesting,
