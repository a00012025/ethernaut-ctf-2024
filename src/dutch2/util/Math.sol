...
library Math {
    ...
    function mul(
        ...
        (bool res, bytes memory ret) = address(0x07).staticcall{gas: 6000}(
    ...
    function availableTokensAtTime(
        uint32 start,
        uint32 end,
        uint32 current,
        uint128 cliff,
        ...
        if (current > end) {
            ...
            return
                uint128(
                    cliffAmount +
                        uint128(amount - cliffAmount).mulDivDown(
                            current - start,
                            end - start
                        )
                );
    ...
    function encrypt(
        ...
        uint256 withPriv,
        bytes32 message
    ) internal view returns (Point memory pub, bytes32 encrypted) {
